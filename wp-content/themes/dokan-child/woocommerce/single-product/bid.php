...
<p class="auction-user mobile_hide"><a href="javascript:"  class="detail_link1">Details</a><!--<span class="close_link" style="display:none;"><a href="javascript:" >X</a></span>--></p>
  <div class="accordion_div1">
    ...
    <p class="auction-user desktop_hide">#auction_no1##</p>
	<p class="auction-user mobile_hide">#auction_no2##</p>
	
  </div></div>';
if($product_id==$demo_listing){
	$html_detail = str_replace('#auction_no1##',"<strong>All Auctions NATIONWIDE</strong>",$html_detail);
	$html_detail = str_replace('#auction_no2##',"<strong>All Auctions NATIONWIDE</strong>",$html_detail);
}else{
	$html_detail = str_replace('#auction_no1##','<a href="javascript:"  class="detail_link"><strong>Auction #</strong></a><span class="mobile_line accordion_div popup" style="display:none;"><strong>Auction #</strong><br />'.$auction_no.'<span class="close_link" style="display:none;"><a href="javascript:" >X</a></span></span>',$html_detail);
	$html_detail = str_replace('#auction_no2##','<strong>Auction #:&nbsp;</strong><span class="mobile_line">'.$auction_no.'</span>',$html_detail);
}
if($user_role =='seller' ) {?>
<style type="text/css">
...
<?php global $class_lang; if(isset($_GET['lang']) && $_GET['lang']=='es'){
			$lang = "es";
			$unit_name = '"Año","Día","Hora","Minuto","Segundo"';
			$units_pl = '"Años","Días","Horas","Minutos","Segundos"';
			$labels  = array("Año","Día","Hora","Minuto","Segundo");
	 }else{
		 	$lang = "en";
			$unit_name = '"Year","Day","Hour","Minute","Second"';
			$units_pl = '"Years","Days","Hours","Minutes","Seconds"';
			$labels  = array("Year","Day","Hour","Minute","Second");
	}?>

<!--<script src="/wp-content/themes/dokan-child/js/en.js"></script>-->
<script src="/wp-content/themes/dokan-child/js/_countdown_beta.js"></script>
<script type="application/javascript">
function CountDownNew2(tillData,type){
	//https://widget.time.is/js?UTC_za00&t=1653657969999
	//time_is_widget.cb(1653658041015,1653657969999,[[0,0,0,0,0,0,0,0,0]])
	//var tillData = Date.parse("Feb 03, 2022 13:00:00 UTC-8");
	//var now = Date.parse(str2+" UTC-8") / 1000;
	time_is_widget.init({
		UTC_za00 : {
							id:"UTC_za00",
							leading_zeros:'on',
							template:'<a href="time.is/" id="time_is_link"></a><div class="main-auction auction-time-countdownNew hasCountdown" data-auctionid="<?php echo $product_id;?>"><span class="countdown_row countdown_show4"><span class="countdown_section">(days)</span><span class="countdown_section">(hours)</span><span class="countdown_section">(minutes)</span><span class="countdown_section">seconds</span></span></div>',
							countdown_to:tillData,
							units:"long",
							units_abbr:["","","","",""],
							unit_names:[<?php echo $unit_name;?>],
							units_pl:[<?php echo $units_pl;?>],
							on_0:'<a href="time.is/" id="time_is_link"></a><div class="main-auction auction-time-countdownNew hasCountdown" data-auctionid="<?php echo $product_id;?>"><span class="countdown_row countdown_show4"><span class="countdown_section"><span class="countdown_amount">0</span><br><?php echo $labels[1];?></span><span class="countdown_section"><span class="countdown_amount">0</span><br><?php echo $labels[2];?></span><span class="countdown_section"><span class="countdown_amount">0</span><br><?php echo $labels[3];?></span><span class="countdown_section"><span class="countdown_amount counter_sec">0</span><br><?php echo $labels[4];?></span></span></div>',
							headline:''
						 },
		});
}
...
/*	var start = '<?php echo microtime(TRUE);?>';
	var now = '<?php date_default_timezone_set('America/Los_Angeles'); echo strtotime(date('Y-m-d H:i:s'));?>';
	var end = '<?php echo microtime(TRUE);?>';
	var time_taken = (parseInt(end) - parseInt(start)) * 1000;
	console.log(parseFloat(time_taken));*/
//$time_taken = round($time_taken,3);
	...
	//console.log(time);
	const str2 = new Date().toLocaleString('en-US', { timeZone: 'America/Los_Angeles' ,hour12: false });
	var now = Date.parse(str2+" UTC-8") / 1000;
	/*if(now > tillData){
		var time =  now - tillData;
	}else{
		var time = tillData - now;
	}*/
	var time = tillData - now;
	//alert(str2);
	// New Code
	//var arr = "2022-03-15 10:30:00".split(/[- :]/);
	/*
	var arr = tillData.split(/[- :]/),
    date = new Date(arr[0], arr[1]-1, arr[2], arr[3], arr[4], arr[5]);
	var countDownDate2 = Date.parse(date)/1000;
	
	const str2 = new Date().toLocaleString('en-US', { timeZone: 'America/Los_Angeles' ,hour12: false });
	var arr2 =str2.replace(",","").replace("/","-").replace("/","-").split(/[- :]/),
    date2 = new Date(arr2[2], arr2[1]-1, arr2[0], arr2[3], arr2[4], arr2[5]);
	var now2 = Date.parse(date2)/1000;
	var time2 = countDownDate2 - now2;
	*/
	jQuery('.auction-time-countdownNew').SAcountdown({
...
if(is_user_logged_in() && $post->post_author == $current_user->ID && ($_auction_relist_expire=='' ||$_auction_relist_expire=='no')){
	...
	if(strtotime($_auction_dates_to) < strtotime($today_date_time_seconds) && strtotime($today_date_time_seconds) >= strtotime($_flash_cycle_end) && ($_auction_current_bid == '' || $_auction_current_bid == 0)){?>
    	<?php
			...
			$secs =  $newtimestamp - strtotime($today_date_time_seconds) - 5;
			?>
			...
			.sgpb-content-1640{display:block !important;}
			</style>
        <?php }?>
    <?php }else{?>
        <style type="text/css">
...
<?php if(($product->is_closed() === FALSE ) && ($product->is_started() === TRUE )){
    ...
    <?php if($_auction_dates_extend == 'yes' && $_auction_extend_counter == 'no'){
			/***************************Auction Extended**********************************/
			$auction_status = '<span id="auction_status"><span class="extend_text">Extended</span></span>';
    ...
    <div class="auction-time" id="countdown">
      <!--<div class="main-auction auction-time-countdownNew" data-time="<?php echo $product->get_seconds_remaining();?>" data-auctionid="<?php echo $product_id ?>" data-format="<?php echo get_option( 'simple_auctions_countdown_format' ) ?>"></div>-->
     	<span id="UTC_za00" class="live"></span>
    </div>
	<?php 
		$_auction_dates_to = get_post_meta($product_id, '_auction_dates_to', true );
	?>
    <script >
		 CountDownNew2('<?php echo strtotime(date("Y-m-d H:i",strtotime($_auction_dates_to)))*1000;?>','<?php echo $auction_detail_class;?>');
		   //setTimeout("updateCounter();",10000);
			 function updateCounter(){
				 jQuery.ajax({	
							url:'<?php echo get_site_url();?>/ajax.php',	
							type:'POST',
							data:{'mode':'updateCounter',"type":"<?php echo $auction_detail_class;?>","product_id":"<?php echo $product_id;?>"},
							beforeSend: function() {},
							complete: function() {
							},
							success:function (data){
								CountDownNew2(data,'<?php echo $auction_detail_class;?>');
							}
	...
	//CountDown();
      	...
      	<div class="auction_detail <?php echo $auction_detail_class?>">
			...
			echo do_shortcode('[sc_embed_player_template1 loops="true" volume="10" fileurl="'.$play_sound.'"]');
			...
			.playBtnContainer .jconfirm.jconfirm-white .jconfirm-bg,.playBtnContainer .jconfirm.jconfirm-light .jconfirm-bg{
  ...
  opacity:1;
						...
						if (timetext == 'Pause') {
							...
							jQuery.confirm({
								...
								onContentReady: function () {
										...
										jQuery(".jconfirm.jconfirm-white .jconfirm-bg, .jconfirm.jconfirm-light .jconfirm-bg").addClass("blurBg");	
        ...
        <?php if(is_user_logged_in() && $post->post_author == $current_user->ID){?>
        	  ...
        	  <div class="price priceBox">
                ...
                <input type="hidden" id="_auction_new_price_field" value="<?php echo $_auction_start_price;?>" />
                    ...
                    <tr>
                      <td>
					  	<?php if($_auction_current_bid == '' || $_auction_current_bid == 0){
								...
								$up_click = ' onclick="updatePrice(\'up\',\'live\');"';
								$down_click = ' onclick="updatePrice(\'down\',\'live\');"';
								/*$up_click = ' onclick="playAudio(\''.home_url('wp-content/uploads/sounds/gun-guns-shotgun-int-close-loa.wav').'\');updatePrice(\'up\',\'live\');"';
								$down_click = ' onclick="playAudio(\''.home_url('wp-content/uploads/sounds/gun-guns-shotgun-int-close-loa.wav').'\');updatePrice(\'down\',\'live\');"';*/
							  }else{
								  $up_click = ' onclick="updatePrice(\'up\',\'live\');"';
								$down_click = ' onclick="updatePrice(\'down\',\'live\');"';
								/*$up_click = ' onclick="playAudio(\''.home_url('wp-content/uploads/sounds/gun-guns-shotgun-int-close-loa.wav').'\');updatePrice(\'up\',\'live\');"';
						...
						?>
                        <span id="seller_update_price"><img id="up_price_btn" src="<?php echo home_url('wp-content/themes/dokan-child/Up.png');?>" <?php echo $up_click;?> alt="up" class="up_img" border="0" style="width:30px;"/><img id="down_price_btn" src="<?php echo home_url('wp-content/themes/dokan-child/Down.png');?>" <?php echo $down_click;?> alt="down" class="down_img" border="0" style="width:30px;"/></span>
                        </td>
                    </tr>
                    <tr>
                      <td><?php if($_auction_maximum_travel_distance < 36){?>
                        <span id="seller_update_distance"><img src="<?php echo home_url('wp-content/themes/dokan-child/Up.png');?>" class="up_img" onclick="updateDistance('up','live');" alt="up" border="0" style="width:30px;"/><img src="<?php echo home_url('wp-content/themes/dokan-child/Down.png');?>" class="down_img" onclick="updateDistance('down','live');" alt="Down" border="0" style="width:30px;" id="down_btn"/></span>
                        <?php }?></td>
                    </tr>
                  </table></td>
               ...
               <form id="auction_form" class="auction_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo $product_id; ?>" >
                ...
                <div class="quantity buttons_added"> 
                	<?php 
						...
						//echo $_auction_current_bid."==".$_auction_start_price;
                  	?>
                  <input type="button" value="+" class="plus" />
                  <input type="hidden" id="bid_value" name="bid_value" data-auction-id="<?php echo esc_attr( $product_id ); ?>"  <?php if ($product->get_auction_sealed() != 'yes'){ ?> value="<?php if($_auction_start_price <=1 && $_auction_current_bid==""){ echo .99;}elseif($_auction_start_price <=1 && $_auction_current_bid ==.99){echo '-1';}else{echo $product->bid_value();} ?>" max="<?php  if($_auction_start_price <=1 && $_auction_current_bid==""){ echo .99;}elseif($_auction_start_price <=1 && $_auction_current_bid ==.99){echo '-1';}else{echo $product->bid_value();}  ?>"  <?php } ?> step="any" size="<?php echo strlen($product->get_curent_bid())+2 ?>" title="bid"  class="input-text  qty bid text left" readonly="readonly">
                ...
                <?php }else{
						...
						if($plan_status=='active'){?>
							...
							<a id="bid_anchor" href="javascript:AjaxTest('<?php echo $already_bid;?>','<?php echo $class_lang;?>');"  onclick='	jQuery("#bid_anchor").css("position","relative").css("z-index","-1");'><img class="bid_now_img bid_button" src="<?php echo home_url().'/bid_now_btn.png';?>" border="0" alt="With each bid the amount is reduced by 3%" style="" /></a>
						<?php }else{
							$dentist_account_status = get_user_meta(get_current_user_id(), 'dentist_account_status', true );
...
<?php }elseif(($product->is_closed() === FALSE ) && ($product->is_started() === FALSE )){?>
		...
		<div class="auction-time future" id="countdown">
      		<!--<div class="auction-time-countdownNew future" data-time="<?php echo $product->get_seconds_to_auction() ?>" data-auctionid="<?php echo $product_id ?>" data-format="<?php echo get_option( 'simple_auctions_countdown_format' ) ?>"></div>-->
            <span id="UTC_za00" class="future"></span>
    	</div>
        <?php 
			global $monday;
			$_auction_dates_from = get_post_meta($product_id, '_auction_dates_from', true );
		?>
        <script >
			//CountDownNew('<?php echo strtotime(date("Y-m-d H:i:s",strtotime($_auction_dates_from)));?>','countdown');
			CountDownNew2('<?php echo strtotime(date("Y-m-d H:i:s",strtotime($_auction_dates_from)))*1000;?>','countdown');
		</script>
              ...
              <tr>
                ...
                <td  class="up_down"><table width="100%" border="0" >
                    <tr>
                      <td>
					  	<?php if($_auction_current_bid == '' || $_auction_current_bid == 0){
								$up_click = ' onclick="updatePrice(\'up\',\'countdown\');"';
								$down_click = ' onclick="updatePrice(\'down\',\'countdown\');"';
							  }else{
								  $up_click = ' onclick="updatePrice(\'up\',\'countdown\');"';
									$down_click = ' onclick="updatePrice(\'down\',\'countdown\');"';
							  }
...
<?php }elseif($product->is_closed() === TRUE){?>
    ...
    <?php if($_auction_current_bid){?>
        ...
        <div class="auction-time" id="countdown">
            ...
            <span id="UTC_za00" data-status="win"></span>
    	</div>
        <script>
			 CountDownNew2('<?php echo strtotime(date("Y-m-d H:i:s",strtotime($_auction_dates_to)))*1000;?>','<?php echo $auction_detail_class;?>');
		</script>
                      ...
                      <tr>
                        ...
                        <td  class="up_down"><table width="100%" border="0" >
                            <tr>
                              <td>
					  	<?php if($_auction_current_bid == '' || $_auction_current_bid == 0){
								$up_click = ' onclick="updatePrice(\'up\',\'ended\');"';
								$down_click = ' onclick="updatePrice(\'down\',\'ended\');"';
							  }else{
								  $up_click = ' onclick="updatePrice(\'up\',\'ended\');"';
								 $down_click = ' onclick="updatePrice(\'down\',\'ended\');"';
							  }
						?>
                        <span id="seller_update_distance"><img src="<?php echo home_url('wp-content/themes/dokan-child/Up.png');?>" <?php echo $up_click;?> alt="up" class="up_img" border="0" style="width:30px;"/><img id="down_price_btn" src="<?php echo home_url('wp-content/themes/dokan-child/Down.png');?>" <?php echo $down_click;?> alt="down" class="down_img" border="0" style="width:30px;"/></span>
                        </td>
                            </tr>
                            <tr>
                              <td><?php if($_auction_maximum_travel_distance < 36){?>
                                <span id="seller_update_distance"><img src="<?php echo home_url('wp-content/themes/dokan-child/Up.png');?>" class="up_img" onclick="updateDistance('up','ended');" alt="up" border="0" style="width:30px;"/><img src="<?php echo home_url('wp-content/themes/dokan-child/Down.png');?>" class="down_img" onclick="updateDistance('down','ended');" alt="Down" border="0" style="width:30px;" id="down_btn"/></span>
                                <?php }?></td>
                            </tr>
                          </table></td>
                        <td class="td_3"><table width="100%" border="0" >
		...
		<?php if($user_role !='seller'){?>
        	...
        	<span class="bid_amount_txt" > 
                ...
                <?php if($_auction_start_price <=1 && $_auction_current_bid !=.99){?><span class="woocommerce-Price-amount amount">$<span class="underline_amt_must">0</span></span><?php }else{?><span class="woocommerce-Price-amount amount"><span class="underline_amt_must">NONE</span></span><?php }?>
                </span>
            </span>
        </div>
    	<?php }?>
        ...
        <!-------------------Auction Ended With Flash to Come ------------------------------------------------------>
        	<?php if(strtotime($today_date_time) <= strtotime($_flash_cycle_start) && strtotime($_flash_cycle_end) > strtotime($today_date_time)){?>
                ...
                <div class="auction-time future future_flash" id="countdown">
  					<!--<div class="main-auction auction-time-countdownNew future future_flash" <?php if($_auction_relist_expire=='yes' || $_GET['action']=='expire'){?> data-status="win"<?php }else{}?> data-time="<?php echo $second; ?>" data-auctionid="<?php echo $product_id ?>" data-format="<?php echo get_option( 'simple_auctions_countdown_format' ) ?>"></div>-->
                    <span id="UTC_za00" class="future future_flash" <?php if($_auction_relist_expire=='yes' || $_GET['action']=='expire'){?> data-status="win"<?php }else{}?>></span>
				</div>
				<script >
					//CountDownNew('<?php echo strtotime(date("Y-m-d H:i:s",strtotime($_flash_cycle_start)));?>','countdown-flash');
					 CountDownNew2('<?php echo strtotime(date("Y-m-d H:i:s",strtotime($_flash_cycle_start)))*1000;?>','countdown-flash');
				</script>
...
<script type="text/javascript">
	...
	jQuery("#price_form").submit(function(e) {
			...
			if (jQuery("#update_distance").hasClass("btn_animate")) {
					...
					jQuery.ajax({
						   ...
						   success: function(data)
						   {
							   ...
							   jQuery("#_auction_new_price_field").html(tmp[2]);
							    jQuery("#_new_price").val(tmp[2]);
							   ...
							   jQuery("#seller_screen_update").val('no');
						   }
						 });
			}else{
				//alert("no updated");
		...
		function updateDistance(dir,status){
			//audio_type.play();
			if(status=='ended'){
				Custom_popup("Error!","Travel Miles may not be changed","col-md-4 col-md-offset-4");
			}else{				
				var auction_with_bid = parseInt(jQuery("#auction_with_bid").val());
		...
		function updatePrice(dir,status){
			//audio_type.play();
			var auction_with_bid = jQuery("#auction_with_bid").val();
			var _new_price = parseInt(jQuery("#_new_price").val());
			var _auction_new_price_field = parseInt(jQuery("#_auction_new_price_field").val());
			
			if(auction_with_bid=='no' && status !="ended"){
				if(dir == 'up'){
					playAudio('<?php echo home_url('wp-content/uploads/sounds/gun-guns-shotgun-int-close-loa.wav')?>');
					_new_price++;
					jQuery("#update_distance").addClass('btn_animate');
					...
					jQuery("#seller_screen_update").val('yes');
				}
				...
				if(dir == 'down'){
						...
						if(status =='countdown'){
							 if(_new_price < 0){
								Custom_popup("Error!","Ask Fee may not be decreased","col-md-4 col-md-offset-4");
							}else{
								playAudio('<?php echo home_url('wp-content/uploads/sounds/gun-guns-shotgun-int-close-loa.wav')?>');
								jQuery("#update_distance").addClass('btn_animate');
								jQuery("#_new_price").val(_new_price);
							...
							if(_new_price < parseInt(_auction_new_price_field)){
								Custom_popup("Error!","Ask Fee may not be decreased","col-md-4 col-md-offset-4");
							}else if(_new_price < 0){
								Custom_popup("Error!","Ask Fee may not be decreased","col-md-4 col-md-offset-4");
							}else{
								playAudio('<?php echo home_url('wp-content/uploads/sounds/gun-guns-shotgun-int-close-loa.wav')?>');
								jQuery("#update_distance").addClass('btn_animate');
				...
				*/
				...
				Custom_popup("Error!","Ask Fee may not be changed","col-md-4 col-md-offset-4");
			}
		}
jQuery(document).ready(function() {
...
<input type="hidden" id="seller_screen_update" value="no" />
<?php }else{?>
