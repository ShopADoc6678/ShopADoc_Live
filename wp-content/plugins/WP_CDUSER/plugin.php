    ...
    <div id="post-body" class="metabox-holder">
      <div id="post-body-content">
        <div class="meta-box-sortables ui-sortable">
          ...
          <?php 
		  $item = CDUSER_List::get_UsersFunc($_POST['firstname'],$_POST['lastname']);
            ...
            <?php foreach($item['user_data'] as $user){
				...
				foreach ($customer_orders as $customer_order) {
					...
					if($userArray->roles[0]=='seller'){
						$Order_Array[0]['popup'] = '<a onClick="openUser(\''. $userArray->ID.'\',\'client\');" href="javascript:"><strong>C</strong></a>';
					}
					if($userArray->roles[0]=='customer'){
						$Order_Array[0]['popup'] = '<a onClick="openUser(\''. $userArray->ID.'\',\'dentist\');" href="javascript:"><strong>D</strong></a>';
					}			
				}
            	...
            	<?php foreach($Order_Array as $order){ ?>
					<?php if($order['order_ref']!=""){?>
                          <tr >
                                ...
                                <td align="center"><?php if($order['Auction_id']!=""){?><a href="admin.php?page=auctions&search_str=<?php echo $order['Auction_id'];?>" title="<?php echo $order['Auction_id'];?>" target="_new"><?php echo $order['Auction_id'];?></a><?php }else{?>-<?php }?></td>
