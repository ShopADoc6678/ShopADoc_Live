...
class ADVERTISER_List extends WP_List_Table {
	 ...
	 * @return mixed
	 */
	public static function get_ADVERTISERs( $searchStr ) {
...
function submitUser(){
  ...
  #toplevel_page_admin-page-ADVERTISER a{
			background: #2271b1 !important;
			color: #fff !important;
			}
			#toplevel_page_admin-page-ADVERTISER a:after {
			right: 0;
  ...
  <?php 
  				...
  				if (isset($_GET["action"]) && $_GET["action"] == 'upgrade') {
					...
					header("location:/wp-admin/admin.php?page=ADVERTISER&update=upgradeUser");
					exit;
				}
				if (isset($_GET["action"]) && $_GET["action"] == 'delete') {
					...
					//$user->add_role( 'advanced_ads_user' );
					$user_id = $_GET['user'];
					$email = "advertiser".strtotime(date("i:s"))."@shopadoc.com";
					$wpdb->update( 'wp_users', array('user_login' => $email, 'user_email' => $email, 'user_status' => '3',) , array( 'ID' => $user_id ) );
					header("location:/wp-admin/admin.php?page=ADVERTISER&update=del");
					exit;
				}
				if (isset($_GET["action"]) && $_GET["action"] == 'approve') {
					update_user_meta($_GET['user'], 'deactivate_advertiser','No');
					header("location:admin.php?page=ADVERTISER&update=approveUser");
					exit;
				}
				if (isset($_GET["action"]) && $_GET["action"] == 'deactive') {
					update_user_meta($_GET['user'], 'deactivate_advertiser','Yes');
					header("location:admin.php?page=ADVERTISER&update=deactiveUser");
					exit;
				}
...
<!--  <?php if (isset($_GET["update"]) && $_GET["update"] == 'update') {?>
  ...
  <?php }?>
  <?php if (isset($_GET["msg"]) && $_GET["msg"] == 'deactiveUser') {?>
  <div id="setting-error-wpforms-deactived" class="notice notice-success settings-error is-dismissible" style="display:block;">
    <p><strong>user deactived.</strong></p>
    <button type="button" class="notice-dismiss" onclick="jQuery('#setting-error-wpforms-deactived').hide();"><span class="screen-reader-text">Dismiss this notice.</span></button>
  </div>
  <?php }?>-->
  <?php 
					$params = array();
					...
					//print_r($params);
					if(trim($params['update'])=='del'){
						echo '<div id="message" class="updated is-dismissible"><p>User deleted.</p></div>';
					}
					if(trim($params['update'])=='del_many'){
						echo '<div id="message" class="updated is-dismissible"><p>User deleted.</p></div>';
					}
					if(trim($params['update'])=='add'){
						echo '<div id="message" class="updated is-dismissible"><p>New user created.</p></div>';
					}
					if(trim($params['update'])=='edit'){
					...
					if(trim($params['update'])=='promote'){
						echo '<div id="message" class="updated is-dismissible"><p>Changed roles.</p></div>';
					}
					if(trim($params['update'])=='err_admin_remove'){
						echo '<div id="message" class="error is-dismissible"><p>You can\'t remove the current user."</p></div>';
					}
        ...
        <div class="meta-box-sortables ui-sortable">
          ...
          <table class="wp-list-table widefat fixed striped table-view-list users">
            ...
            <tbody id="the-list" data-wp-lists="list:user">
			  ...
			  ?>
              <tr id="user-<?php echo $user->ID;?>" data-id="<?php echo $user->ID;?>" <?php if($deactivate_advertiser=='Yes'){?>style="background-color: rgb(255, 255, 224);"<?php }?>>
                <td class="username has-row-actions column-primary" data-colname="Username"><strong><!--<a href="/wp-admin/user-edit.php?user_id=<?php echo $user->ID;?>&wp_http_referer=/wp-admin/admin.php?page=ADVERTISER">--><a href="javascript:addUser('<?php echo $user->ID;?>','');"><?php echo $ad_demo_company_name;?></a></strong><br>
                  <div class="row-actions"> <span class="edit"><a href="javascript:addUser('<?php echo $user->ID;?>','');">Edit</a> | </span> 
                  <span class="delete"><a class="submitdelete" href="<?php echo wp_nonce_url('admin.php?page=ADVERTISER&action=delete&user='.$user->ID);?><?php //echo wp_nonce_url( "users.php?action=delete&user=".$user->ID."&wp_http_referer=/wp-admin/admin.php?page=ADVERTISER&update=del", 'bulk-users' );?>">Delete</a> | </span>
...
<?php
	...
	public function screen_option() {
		...
		$this->stats_obj = new ADVERTISER_List();
	}
...
add_action( 'plugins_loaded', function () {
	SP_Plugin_ADVERTISER::get_instance();
} );